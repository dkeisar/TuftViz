...
%
%   weightVector = fmin_adam(@(theta)labelingMSEGradients(theta, X, label), theta0, 0.01)
%
...
if (max(isinf(fval0)) || max(isnan(fval0)) || max(any(isinf(vfCdX0))) || max(max(isnan(vfCdX0))))
